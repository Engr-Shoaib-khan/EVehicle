...
class _RideHistoryScreenState extends State<RideHistoryScreen> {
  ...
  Future<void> _loadHistory() async {
    ...
    try {
      ...
      if (res.statusCode == 200) {
        ...
        setState(
            () => _rides = List<Map<String, dynamic>>.from(data['data'] ?? []));
      } else {
  ...
  Widget _buildEmpty() => const Center(
        ...
        Text('Your completed and cancelled rides\nwill appear here.',
            ...
            style: TextStyle(
                fontSize: 13.0, color: Color(0xFF6B7280), height: 1.5)),
      ]));

  Widget _buildError() => Center(
          child: Column(mainAxisAlignment: MainAxisAlignment.center, children: [
        const Icon(Icons.wifi_off_rounded,
            color: Color(0xFF9CA3AF), size: 44.0),
...
class _RideTile extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final pickup =
        (ride['pickupLocation'] as Map?)?['address'] as String? ?? 'Pickup';
    final dropoff =
        (ride['dropoffLocation'] as Map?)?['address'] as String? ?? 'Drop-off';
    ...
    return GestureDetector(
      onTap: completed
          ? () => Navigator.push(context,
              MaterialPageRoute(builder: (_) => ReceiptScreen(ride: ride)))
          : null,
      child: Container(
        padding: const EdgeInsets.all(16.0),
                ...
                child: Column(
                  ...
                  Text(dateStr,
                      style: const TextStyle(
                          fontSize: 11.0, color: Color(0xFF9CA3AF))),
                ])),
            if (completed) ...[
              Text('Rs. $finalFare',
                  style: const TextStyle(
          ...
          if (driverName != '—') ...[
            ...
            Row(children: [
              const Icon(Icons.person_outline_rounded,
                  size: 13.0, color: Color(0xFF9CA3AF)),
...
class _RouteChip extends StatelessWidget {
  ...
  Widget build(BuildContext context) => Row(children: [
        Icon(icon, color: color, size: 10.0),
        const SizedBox(width: 6.0),
        Expanded(
            child: Text(label,
                style:
                    const TextStyle(fontSize: 12.0, color: Color(0xFF0D1B2A)),
                maxLines: 1,

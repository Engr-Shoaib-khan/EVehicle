...
// ── Map style ─────────────────────────────────────────────────────
const String _kStyle = '[{"featureType":"poi","elementType":"labels","stylers":[{"visibility":"off"}]},{"featureType":"road","elementType":"geometry","stylers":[{"color":"#ffffff"}]},{"featureType":"water","elementType":"geometry","stylers":[{"color":"#c9e8f0"}]},{"featureType":"landscape","elementType":"geometry","stylers":[{"color":"#f0f4f0"}]}]';
...
class _ActiveRideScreenState extends State<ActiveRideScreen>
  ...
  void initState() {
    ...
    if (widget.driver != null && widget.driver!.location != const LatLng(0, 0)) {
  ...
  void _onStatus(Map<String, dynamic> d) {
    ...
    if (status == 'driver_arrived') setState(() => _driverStep = _DriverStep.arrived);
    if (status == 'in_progress')    setState(() => _driverStep = _DriverStep.inProgress);
    if (status == 'completed')      setState(() => _driverStep = _DriverStep.completed);
  }
  ...
  void _onSos(Map<String, dynamic> d) {
    showDialog(context: context, builder: (_) => AlertDialog(
      ...
      title: const Row(children: [
        ...
        SizedBox(width: 8.0),
        Text('SOS Alert', style: TextStyle(color: Color(0xFFEF4444), fontWeight: FontWeight.w700)),
      ]),
      content: Text(d['message'] as String? ?? 'Emergency alert triggered.'),
  ...
  Future<void> _advanceStep() async {
    ...
    try {
      ...
      final result = await _rideService.updateRideStatus(widget.rideId, newStatus);
  ...
  void _placeDriverMarker(LatLng pos) {
    ...
    _markers.add(Marker(
      markerId: const MarkerId('driver'), position: pos,
  ...
  void _triggerSos() {
    ...
    showDialog(context: context, builder: (_) => AlertDialog(
      shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(18.0)),
      title: const Text('Emergency', style: TextStyle(fontWeight: FontWeight.w700, color: Color(0xFFEF4444))),
      content: const Text('Choose an emergency action:'),
      actions: [
        TextButton.icon(
          icon: const Icon(Icons.call, color: Color(0xFFEF4444)),
          label: const Text('Call Police (15)', style: TextStyle(color: Color(0xFFEF4444), fontWeight: FontWeight.w700)),
          onPressed: () {
  ...
  Map<String, dynamic> get _stepConfig {
    switch (_driverStep) {
      case _DriverStep.heading:
        return {'label': 'I Have Arrived', 'icon': Icons.location_on_rounded, 'color': const Color(0xFF1565C0)};
      case _DriverStep.arrived:
        return {'label': 'Start Trip', 'icon': Icons.play_arrow_rounded, 'color': kGreen};
      case _DriverStep.inProgress:
        return {'label': 'End Trip', 'icon': Icons.flag_rounded, 'color': const Color(0xFFEF4444)};
      default:
        return {'label': 'Completed', 'icon': Icons.check_rounded, 'color': kGreen};
    }
  }
                ...
                child: Column(mainAxisSize: MainAxisSize.min, children: [
                  ...
                  Container(
                    ...
                    child: Row(mainAxisAlignment: MainAxisAlignment.center, children: [
                      Icon(isDone ? Icons.check_circle_rounded : Icons.bolt_rounded,
                  ...
                  Padding(
                    ...
                    child: Column(children: [
                      ...
                      if (widget.driver != null)
                        Row(children: [
                          Container(
                            ...
                            child: Center(child: Text(widget.driver!.fullName[0].toUpperCase(),
                          ...
                          Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                            Text(widget.driver!.fullName,
                                style: const TextStyle(fontSize: 14.0, fontWeight: FontWeight.w700, color: Color(0xFF0D1B2A))),
                            Row(children: [
                              const Icon(Icons.star_rounded, color: Colors.amber, size: 13.0),
                              const SizedBox(width: 2.0),
                              Text('${widget.driver!.rating}  ·  ${widget.driver!.vehicleMake} ${widget.driver!.vehicleModel}',
                                  style: const TextStyle(fontSize: 11.0, color: Color(0xFF6B7280))),
                            ]),
                          ])),
                          Column(crossAxisAlignment: CrossAxisAlignment.end, children: [
                            Text(widget.driver!.plateNumber,
                                style: const TextStyle(fontSize: 13.0, fontWeight: FontWeight.w700, color: Color(0xFF0D1B2A))),
                        ...
                        const Row(children: [
                          ...
                          SizedBox(width: 16),
                          Text('Searching for nearby drivers...', style: TextStyle(fontWeight: FontWeight.w600)),
                        ]),
                      ...
                      Container(
                        ...
                        child: Row(children: [
                          Icon(Icons.battery_charging_full_rounded, color: _socColor, size: 16.0),
                          const SizedBox(width: 8.0),
                          const Text('Vehicle Battery',
                              style: TextStyle(fontSize: 11.0, color: Color(0xFF6B7280))),
                          const Spacer(),
                          Text('${_soc.toStringAsFixed(0)}%',
                              style: TextStyle(fontSize: 13.0, fontWeight: FontWeight.w700, color: _socColor)),
                          const SizedBox(width: 8.0),
                          SizedBox(
                            width: 72.0, height: 7.0,
                            child: ClipRRect(
                      ...
                      if (_soc <= 15) ...[
                        ...
                        Container(
                          ...
                          decoration: BoxDecoration(color: const Color(0xFFFEF2F2), borderRadius: BorderRadius.circular(10.0)),
                          child: const Row(children: [
                            Icon(Icons.warning_rounded, color: Color(0xFFEF4444), size: 14.0),
                            SizedBox(width: 6.0),
                            Expanded(child: Text('Battery critical! Navigate to nearest EV charging station.',
                                style: TextStyle(fontSize: 11.0, color: Color(0xFFEF4444)))),
                          ]),
                        ),
                      ],
                      ...
                      // ── Route ────────────────────────────────
                      _RouteRow(icon: Icons.circle, color: kGreen, label: 'From', value: widget.pickupAddress ?? 'Pickup'),
                      const SizedBox(height: 5.0),
                      _RouteRow(icon: Icons.location_on_rounded, color: const Color(0xFFEF4444),
                          label: 'To', value: widget.dropoffAddress ?? 'Dropoff'),
                      ...
                      Row(children: [
                        ...
                        GestureDetector(
                          ...
                          child: Container(
                            ...
                            child: const Column(mainAxisAlignment: MainAxisAlignment.center, children: [
                              ...
                              Text('SOS', style: TextStyle(color: Color(0xFFEF4444), fontSize: 9.0, fontWeight: FontWeight.w800)),
                            ]),
                          ),
                        ),
                        ...
                        GestureDetector(
                          ...
                          child: Container(
                            ...
                            child: const Column(mainAxisAlignment: MainAxisAlignment.center, children: [
                              Icon(Icons.share_location_rounded, color: kGreenDark, size: 20.0),
                              Text('Share', style: TextStyle(color: kGreenDark, fontSize: 9.0, fontWeight: FontWeight.w800)),
                            ]),
                          ),
                        ),
                        const SizedBox(width: 10.0),
                          ...
                          child: widget.isDriverMode
                              ? SizedBox(
                                  ...
                                  child: ElevatedButton(
                                    onPressed: (isDone || _actionLoading) ? null : _advanceStep,
                                    ...
                                    child: _actionLoading
                                        ...
                                        : Row(mainAxisAlignment: MainAxisAlignment.center, children: [
                                            Icon(cfg['icon'] as IconData, size: 18.0),
                                            const SizedBox(width: 6.0),
                                            Flexible(child: Text(cfg['label'] as String,
                              ...
                              : Container(
                                  height: 52.0,
                                  decoration: BoxDecoration(color: const Color(0xFFE3F2FD), borderRadius: BorderRadius.circular(14.0)),
                                  child: const Row(mainAxisAlignment: MainAxisAlignment.center, children: [
                                    Icon(Icons.phone_rounded, color: Color(0xFF1565C0), size: 20.0),
                                    SizedBox(width: 6.0),
                                    Text('Call Driver', style: TextStyle(color: Color(0xFF1565C0), fontSize: 13.0, fontWeight: FontWeight.w700)),
                                  ]),
                                ),
                        ),
                      ]),
...
class _RouteRow extends StatelessWidget {
  ...
  Widget build(BuildContext context) => Row(children: [
    Icon(icon, color: color, size: 11.0), const SizedBox(width: 8.0),
    Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
      Text(label, style: const TextStyle(fontSize: 10.0, color: Color(0xFF6B7280))),

...
class _KycUploadScreenState extends State<KycUploadScreen> {
  ...
  Future<void> _submit() async {
    if (_cnicFront == null ||
        ...
        _license == null ||
        _vehicleReg == null) {
      _showSnack('Please upload all 4 documents before submitting.',
          isError: true);
      return;
    }
    if (_cnicCtrl.text.trim().isEmpty || _licenseCtrl.text.trim().isEmpty) {
      _showSnack('CNIC and License Number are required.', isError: true);
      return;
    }
    ...
    try {
      ...
      final req =
          http.MultipartRequest('POST', Uri.parse('$kBaseUrl/api/kyc/upload'));
      req.headers['Authorization'] = 'Bearer $token';
      req.fields['cnic'] = _cnicCtrl.text.trim();
      req.fields['licenseNumber'] = _licenseCtrl.text.trim();
      req.files.add(
          await http.MultipartFile.fromPath('cnic_front', _cnicFront!.path));
      req.files
          .add(await http.MultipartFile.fromPath('cnic_back', _cnicBack!.path));
      req.files
          .add(await http.MultipartFile.fromPath('license', _license!.path));
      req.files.add(
          await http.MultipartFile.fromPath('vehicle_reg', _vehicleReg!.path));
                      ...
                      children: [
                        ...
                        Container(
                          ...
                          child: const Row(children: [
                            Icon(Icons.verified_user_outlined,
                                color: kGreenDark, size: 18.0),
                            SizedBox(width: 10.0),
                            Expanded(
                                child: Text(
                                    'Upload clear photos of all documents. Approval takes up to 24 hours.',
                                    style: TextStyle(
                                        fontSize: 12.0,
                                        color: kGreenDark,
                                        height: 1.4))),
                        ...
                        TextField(
                          ...
                          style: const TextStyle(
                              fontSize: 14.0, color: Color(0xFF0D1B2A)),
                          decoration: _inputDeco(
                              'e.g. 42101-1234567-1', Icons.badge_outlined),
                        ),
                        ...
                        TextField(
                          ...
                          style: const TextStyle(
                              fontSize: 14.0, color: Color(0xFF0D1B2A)),
                          decoration: _inputDeco(
                              'e.g. LHR-12345678', Icons.credit_card_rounded),
                        ),
                        const SizedBox(height: 22.0),
    ...
    return Padding(
      ...
      child: Column(mainAxisAlignment: MainAxisAlignment.center, children: [
        Container(
          ...
          child:
              const Icon(Icons.check_rounded, color: Colors.white, size: 40.0),
        ),
        const SizedBox(height: 20.0),
        ...
        Text(_successMsg!,
            ...
            style: const TextStyle(
                fontSize: 14.0, color: Color(0xFF6B7280), height: 1.5)),
        const SizedBox(height: 28.0),
        SizedBox(
            width: double.infinity,
            height: 52.0,
  ...
  InputDecoration _inputDeco(String hint, IconData icon) => InputDecoration(
        ...
        focusedBorder: OutlineInputBorder(
            borderRadius: BorderRadius.circular(12.0),
            borderSide: const BorderSide(color: kGreen, width: 2.0)),
        contentPadding:
    ...
    return GestureDetector(
      ...
      child: Container(
        ...
        decoration: BoxDecoration(
          ...
          borderRadius: BorderRadius.circular(14.0),
          border: Border.all(
              color: uploaded ? kGreen : const Color(0xFFE5E7EB),
              width: uploaded ? 1.8 : 1.0),
          boxShadow: [
            BoxShadow(
            ...
            child: uploaded
                ? ClipRRect(
                    ...
                    child: Image.file(file!,
                        fit: BoxFit.cover, width: 44.0, height: 44.0))
              ...
              child: Column(
                ...
                Text(uploaded ? 'Tap to change photo' : 'Tap to upload photo',
                    style: TextStyle(
                        fontSize: 11.0,
                        color:
                            uploaded ? kGreenDark : const Color(0xFF9CA3AF))),
              ])),
          Icon(
              uploaded
                  ? Icons.check_circle_rounded
                  : Icons.add_photo_alternate_outlined,
              color: uploaded ? kGreen : const Color(0xFF9CA3AF),
              size: 22.0),
        ]),
      ),
    );
